...
int main()
{
	...
	names = mcrypt_list_algorithms(&jmax);
	modes = mcrypt_list_modes(&imax);
	...
	for (j = 0; j < jmax; j++) {
		...
		if (mcrypt_module_self_test(names[j]) == 0) {
			printf("ok\n");
		} else {
			x=1;
			printf("\n");
		...
		printf("Modes:\n");
			for (i=0;i<imax;i++) {
				td = mcrypt_module_open(names[j], modes[i]);
				td2 = mcrypt_module_open(names[j], modes[i]);
				if (td != MCRYPT_FAILED && td2 != MCRYPT_FAILED) {
					...
					if (mcrypt_enc_is_block_mode(td)!=0)
						size = (strlen(TEXT) / mcrypt_enc_get_block_size(td))*mcrypt_enc_get_block_size(td);
					else size = strlen(TEXT);

					text = calloc(1, size);
					if (text==NULL) exit(1);

					memmove(text, TEXT, size);
					...
					mdecrypt_generic(td2, text, size);
					if (memcmp(text, TEXT, size) == 0) {
						printf("   %s: ok\n", modes[i]);
					} else {
						printf("   %s: failed\n", modes[i]);
						x=1;
